...
namespace BloodDonationSupport.Domain.Users.Entities
{
    public class User : AggregateRoot<long>
    {
        ...
        private User(Email email, string cognitoUserId, string phoneNumber)
        {
            Email = email;
        ...
        public static User RegisterNewUser(Email email, string cognitoUserId, bool emailExists, string? phone = null)
        {
            UniqueEmailRule.Check(emailExists);
            return new User(email, cognitoUserId, phone);
        }
